...
import NewDialog from "./commons/dialogs/NewDialog";
...
class Stock extends Component {
  ...
  handleSwitchActivate(id) {
    ...
    this.Requests.query(pathQuery + id).then(response => {
      ...
      const pathActDeactivate =
        (this.state.activeTab === 0 ? "/categoria/" : "/articulo/") +
        (response.estado && response.estado == 1 ? "deactivate" : "activate");
      this.Requests.ActivateOrDeactivate(pathActDeactivate, id).then(
        response => {
          this.loadData();
        }
      );
  ...
  handleOnEdit(data) {
    const path =
      this.state.activeTab === 0 ? "/categoria/update" : "/articulo/update";
  ...
  handleRegister(data) {
    const path =
      this.state.activeTab === 0 ? "/categoria/add" : "/articulo/add";
  ...
  loadData() {
    ...
    return this.Requests.list(path).then(response => {
      ...
      if (response && response.length !== 0) {
        // El request no ha devuelto un arreglo vacio
        let dt = response;
          dt.map((el, i) => {
